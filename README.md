...
- **Fifteen color schemes** — Catppuccin, Gruvbox, Dracula, Tokyo Night, Monokai Pro, Atom One, Everforest and Nord, in dark/light pairs wherever upstream publishes both. The same palettes `cps theme` deploys to kitty, so the web terminal and the native one match. Switched from the control panel and remembered per browser. See [Theme and fonts](#theme-and-fonts).
...
- **Themes:** fifteen schemes in the control panel's **Theme** dropdown, in dark/light pairs wherever upstream publishes both — Catppuccin Mocha (the default) and Latte, Gruvbox Dark and Light, Dracula and Alucard, Tokyo Night and Tokyo Night Day, Monokai Pro and Monokai Pro Light, Atom One Dark and Light, Everforest Dark and Light, and Nord, which is dark-only upstream. Each recolors the page and the terminal's ANSI palette together, and the choice is remembered in the browser's `localStorage` — it is per-browser, not a server setting, so two people on the same rinnegan can run different themes. A program that draws in ANSI 0–15 follows the dropdown; one that hardcodes its own colors (most vim colorschemes, anything emitting 24-bit escapes) does not.
- **Palette:** every scheme is generated by `scripts/gen-themes.mjs` from the kitty theme of the same name in [`Tanq16/cli-Productivity-Suite`](https://github.com/Tanq16/cli-Productivity-Suite), so `cps theme <name>` and this dropdown paint the same terminal. Background, foreground, cursor, selection and all sixteen ANSI slots are taken verbatim, including that repo's fixes to them: the bright slots carry a hue-adjacent accent instead of a copy of the normal, and `color7`/`color15` are un-inverted so the brighter grey is the bright one. Colors 16–255 are the fixed xterm cube and are not derived from the sixteen, so those slots are the whole of what a theme can move. Panel tones and status roles have no kitty counterpart and are derived from the palette rather than chosen: recessed surfaces on a fixed perceptual ladder off the background, and status colors from the palette's own hue, reaching for the bright twin only where the normal one cannot be read on a panel. Where a scheme has no further accent to give — Monokai Pro, Everforest, Atom One and Nord — the repeated bright slot is kept rather than invented around. Bold cells are not brightened (matching kitty); true 24-bit color is enabled end to end.
- **Regenerating:** `node scripts/gen-themes.mjs <cps-checkout>/internal/configs/themes css` prints the blocks for `public/styles.css`; `html` prints the dropdown, and no argument prints a contrast audit of every UI role in every theme.
