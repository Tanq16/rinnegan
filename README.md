...
- **Browser proxy to host HTTP servers** — open anything serving HTTP on the box at `/_rinnegan/proxy/<port>/`, in a tab, with no client to install. See [Proxy](#proxy).
...
Every rinnegan route lives under a **`/_rinnegan`** prefix — the login page, the terminal WebSocket, transfers, the tunnel, its own assets, and the proxy. Only `/` sits outside, as the entry point. That leaves the whole rest of the URL space free for [proxied apps](#proxy), so an upstream serving its own `/app.js` or `/ws` can never collide with rinnegan's.
...
| `cookie.refreshTtlSeconds` | `604800` | Refresh cookie lifetime (scoped to `/_rinnegan/refresh`); minimum 60 |
...
| `proxy.enabled` | `true` | Serves `/_rinnegan/proxy/<port>/`; `false` removes the routes and hides the panel section. See [Proxy](#proxy) |
...
Anything serving **HTTP on the host** is reachable from your browser at `/_rinnegan/proxy/<port>/`. Start a server on the box — a static file server for a design you want to look at, a dev server for the frontend you are building, a notebook, a preview of a rendered file — and open it in a tab. It is gated by the same session as the terminal, so nothing new is exposed to the internet. The control panel's Proxy section is a shortcut: type a port, press Enter, and it opens in a new tab.

There is nothing to set up and nothing to start. `/_rinnegan/proxy/…` is a route on the server rinnegan already runs, so it costs nothing when idle: each request connects to `127.0.0.1:<port>` at that moment, pipes the bytes back, and is done. If nothing is listening you get a `502` and no lingering state — start the service later and the next request just works.
...
- **Palette:** Mocha's hex values are taken from the kitty config in [`Tanq16/cli-Productivity-Suite`](https://github.com/Tanq16/cli-Productivity-Suite) so the web terminal matches the native setup, and the other seven reproduce their upstream terminal palettes exactly. A few UI surface tones are chosen rather than sourced, where a scheme defines nothing darker than its own background. Upstream contrast quirks are reproduced rather than corrected — Catppuccin Latte's yellow status text and its salmon selection are both genuinely low-contrast, and that is Latte, not a bug. Bold cells are not brightened (matching kitty); true 24-bit color is enabled end to end.
...
- **The [proxy](#proxy) grants no authority the shell does not already grant** — it reaches host-local HTTP servers, which a shell can do anyway — but proxied pages run on rinnegan's origin and so can reach its routes with your session. Proxy only what you run yourself, or set `proxy.enabled: false`.
