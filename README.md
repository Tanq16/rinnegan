...
- **Eleven dark color schemes** — Catppuccin Mocha, Gruvbox, Dracula, Nord, Atom One, Tokyo Night, Everforest, Kanagawa, Monokai, Rosé Pine, and Solarized, switched from the control panel and remembered per browser, each with all sixteen ANSI slots distinct. See [Theme and fonts](#theme-and-fonts).
...
- **Themes:** eleven schemes in the control panel's **Theme** dropdown — Catppuccin Mocha (the default), Gruvbox, Dracula, Nord, Atom One, Tokyo Night, Everforest, Kanagawa, Monokai, Rosé Pine, and Solarized. All eleven are dark, so none carries a "Dark" in its name; rinnegan ships no light scheme. Each recolors the page and the terminal's ANSI palette together, and the choice is remembered in the browser's `localStorage` — it is per-browser, not a server setting, so two people on the same rinnegan can run different themes. A program that draws in ANSI 0–15 follows the dropdown; one that hardcodes its own colors (most vim colorschemes, anything emitting 24-bit escapes) does not.
- **Palette:** Mocha's hex values are taken from the kitty config in [`Tanq16/cli-Productivity-Suite`](https://github.com/Tanq16/cli-Productivity-Suite) so the web terminal matches the native setup, and the other ten come from their upstream terminal palettes. All sixteen ANSI slots hold a distinct color in every theme: most schemes publish one value per hue and repeat it in the bright slot, which throws away six of the sixteen, so those brights are lightened from the normal in OKLab — same hue, more light, gamut-clamped. Colors 16–255 are the fixed xterm cube and are not derived from the sixteen, so this is the whole of what a theme can move. A few UI surface tones are chosen rather than sourced, where a scheme defines nothing darker than its own background. Upstream quirks are kept rather than corrected — Solarized spends its bright slots on its base greys, so its bright green is a grey, and Rosé Pine puts a teal in the green slot; both are faithful, not broken. The one exception is Solarized's bright black, which upstream sets to the background itself: it is lifted onto the same base02–base01 grey axis, because a slot that renders as nothing is a lost slot, not a quirk. Bold cells are not brightened (matching kitty); true 24-bit color is enabled end to end.
